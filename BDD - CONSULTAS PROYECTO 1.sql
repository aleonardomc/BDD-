...
from [INSTANCIA_SALES].AW_Sales.Sales.SalesOrderHeader soh
inner join
(select salesorderid, productid, orderqty, linetotal
from [INSTANCIA_SALES].AW_Sales.Sales.SalesOrderDetail sod
where ProductID in (
	select ProductID
	from [INSTANCIA_PRODUCTION].AW_Production.Production.Product
	where ProductSubcategoryID in (
		select ProductSubcategoryID
		from [INSTANCIA_PRODUCTION].AW_Production.Production.ProductSubcategory
		where ProductCategoryID in(
			select ProductCategoryID
			from [INSTANCIA_PRODUCTION].AW_Production.Production.ProductCategory	
...
create or alter procedure consultaB as
	...
	(select * from [INSTANCIA_SALES].AW_Sales.Sales.SalesOrderHeader) as A
	inner join
	(select *  from [INSTANCIA_SALES].AW_Sales.Sales.SalesTerritory where TerritoryID between '1' and '6') as B
	on A.TerritoryID = B.TerritoryID
	inner join
	(select * from [INSTANCIA_SALES].AW_Sales.Sales.SalesOrderDetail) as C
	on A.SalesOrderID = C.SalesOrderID
	inner join
	(select * from [INSTANCIA_PRODUCTION].AW_Production.Production.Product) as D
...
begin
	...
	(select * from [INSTANCIA_PRODUCTION].AW_Production.Production.[Location] where LocationID = @idLocation) as A
	inner join
	(select * from [INSTANCIA_PRODUCTION].AW_Production.Production.ProductInventory) as B
	on B.LocationID = A.LocationID
	inner join
	(select * from [INSTANCIA_PRODUCTION].AW_Production.Production.Product) as C
	on B.ProductID = C.ProductID
	inner join
	(select * from [INSTANCIA_PRODUCTION].AW_Production.Production.ProductSubcategory where ProductCategoryID = @idCategoria) as D
	...
	begin
		update [INSTANCIA_PRODUCTION].AW_Production.Production.ProductInventory
		set Quantity = floor(Quantity*1.05)
		where LocationID = @idLocation and ProductID in (
		select B.ProductID from (
			(select * from [INSTANCIA_PRODUCTION].AW_Production.Production.Product) as B
			inner join
			(select * from [INSTANCIA_PRODUCTION].AW_Production.Production.ProductSubcategory) as C
			on B.ProductSubcategoryID = C.ProductSubcategoryID))
			select * from [INSTANCIA_PRODUCTION].AW_Production.Production.ProductSubcategory where ProductSubcategoryID = 1
	end
end


select * from [INSTANCIA_PRODUCTION].AW_Production.Production.ProductInventory
exec consultaC 2,60
...
begin
	...
	(select * from [INSTANCIA_SALES].AW_Sales.Sales.Customer) as A
	inner join
	(select * from [INSTANCIA_SALES].AW_Sales.Sales.SalesTerritory) as B
	on A.TerritoryID = B.TerritoryID
	inner join
	(select * from [INSTANCIA_SALES].AW_Sales.Sales.SalesOrderHeader) as C
	...
	where A.TerritoryID != C.TerritoryID)
		...
		print 'Si existen'
		select * from [INSTANCIA_SALES].AW_Sales.Sales.Customer
end
...
begin 
	if not exists (select * from [INSTANCIA_SALES].AW_Sales.Sales.SalesOrderDetail where ProductId = @idProducto)
		print 'Error: Este producto no existe'
	else 
		begin
			if not exists (select * from [INSTANCIA_SALES].AW_Sales.Sales.SalesOrderDetail where SalesOrderID = @idOrden and ProductID = @idProducto)
				print 'Error: Verifica el numero de orden'
			else
				begin
					update [INSTANCIA_SALES].AW_Sales.Sales.SalesOrderDetail
					set OrderQty = @cantidad
					where SalesOrderID = @idOrden and ProductID = @idProducto
					select * from [INSTANCIA_SALES].AW_Sales.Sales.SalesOrderDetail where SalesOrderDetailID = 43659
				end
		end
end

select * from [INSTANCIA_SALES].AW_Sales.Sales.SalesOrderDetail 
...
begin
	if not exists(select * from [INSTANCIA_SALES].AW_Sales.Sales.SalesOrderHeader where SalesOrderID = @idOrden)
		print 'Error: No existe dicha orden '
	else
	begin
		update [INSTANCIA_SALES].AW_Sales.Sales.SalesOrderHeader
		set ShipMethodID = @metodoEnvio
		where SalesOrderID = @idOrden
		print 'El metodo de envio se actualizo correctamente'
		select ShipMethodID from [INSTANCIA_SALES].AW_Sales.Sales.SalesOrderHeader where SalesOrderID = 43695
	end
end

select * from [INSTANCIA_SALES].AW_Sales.Sales.SalesOrderHeader
...
begin
	if exists (
		select * from [INSTANCIA_PERSON].AW_Person.Person.EmailAddress 
		where BusinessEntityID in(
				select BusinessEntityID 
				from [INSTANCIA_PERSON].AW_Person.Person.Person as A
				where A.FirstName=@nombre and A.LastName=@apellido))
	begin
		update [INSTANCIA_PERSON].AW_Person.Person.EmailAddress set EmailAddress = @correo
			where BusinessEntityID in(
